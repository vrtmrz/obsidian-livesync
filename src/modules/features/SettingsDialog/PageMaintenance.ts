...
import { REMOTE_COUCHDB, REMOTE_MINIO, DEFAULT_SETTINGS, type ObsidianLiveSyncSettings, type FilePathWithPrefix, type FilePath, type LoadedEntry, type MetaEntry, type DocumentID, FLAGMD_REDFLAG3_HR, FLAGMD_REDFLAG2_HR, FLAGMD_REDFLAG } from "@/lib/src/common/types.ts";
...
export function pageMaintenance(
    ...
    // Emergency Actions
    void addPanel(pageEl, "Emergency").then((pageEl) => {
        new Setting(pageEl)
            .setName($msg("maintenanceAndRecovery.emergencyActions.lockServer.title"))
            .setDesc($msg("maintenanceAndRecovery.emergencyActions.lockServer.desc"))
            .addButton((button) =>
                button
                    .setButtonText("Lock")
                    .setDisabled(false)
        ...
        new Setting(pageEl)
            .setName($msg("maintenanceAndRecovery.emergencyActions.restart.title"))
            .setDesc($msg("maintenanceAndRecovery.emergencyActions.restart.desc"))
            .addButton((button) =>
                button
