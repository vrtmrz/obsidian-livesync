...
import { balanceChunkPurgedDBs, checkSyncInfo, isCloudantURI, purgeUnreferencedChunks } from "../../../lib/src/pouchdb/utils_couchdb.ts";
...
type OnSavedHandler<T extends AllSettingItemKey> = {
    key: T, handler: OnSavedHandlerFunc<T>,
}

function getLevelStr(level: ConfigLevel) {
    return level == LEVEL_POWER_USER ? " (Power User)" : level == LEVEL_ADVANCED ? " (Advanced)" : level == LEVEL_EDGE_CASE ? " (Edge Case)" : "";
}
...
export class ObsidianLiveSyncSettingTab extends PluginSettingTab {
    ...
    async saveSettings(keys: (AllSettingItemKey)[]) {
        ...
        // if (runOnSaved) {
        const handlers = this.onSavedHandlers.filter(e => appliedKeys.indexOf(e.key) !== -1).map(e => e.handler(this.editingSettings[e.key as AllSettingItemKey]));
    ...
    computeAllLocalSettings(): Partial<OnDialogSettings> {
        const syncMode = this.editingSettings?.liveSync ? "LIVESYNC" : this.editingSettings?.periodicReplication ? "PERIODIC" : "ONEVENTS";
        return {
            syncMode
        }
    }
    ...
    createEl<T extends keyof HTMLElementTagNameMap>(el: HTMLElement, tag: T, o?: string | DomElementInfo | undefined, callback?: ((el: HTMLElementTagNameMap[T]) => void), func?: OnUpdateFunc) {
        const element = el.createEl(tag, o, callback);
        if (func) this.handleElement(element, func);
        return element;
    }

    addEl<T extends keyof HTMLElementTagNameMap>(el: HTMLElement, tag: T, o?: string | DomElementInfo | undefined, callback?: ((el: HTMLElementTagNameMap[T]) => void), func?: OnUpdateFunc) {
            ...
            for (const k of keys) {
                ...
                if (isObjectDifferent(newConf[k], this.initialSettings?.[k])) {
                    ...
                    if (this.isDirty(k as AllSettingItemKey)) {
                        // And modified.
                        this.plugin.confirm.askInPopup(`config-reloaded-${k}`, `The setting "${getConfName(k as AllSettingItemKey)}" being in editing has been changed from somewhere. We can discard modification and reload by clicking {HERE}. Click elsewhere to ignore changes`, (anchor) => {
                            anchor.text = "HERE";
...
Store only the settings. **Caution: This may lead to data corruption**; database reconstruction is generally necessary.`;
            const buttons = [
                OPTION_FETCH, OPTION_REBUILD_BOTH, // OPTION_REBUILD_REMOTE,
                OPTION_ONLY_SETTING, OPTION_CANCEL];
            ...
            if (result == OPTION_FETCH) {
                if (!await checkWorkingPassphrase()) {
                    if (await this.plugin.confirm.askYesNoDialog("Are you sure to proceed?", { defaultOption: "No" }) != "yes") return;
                }
            }
        ...
        void addPane(containerEl, "Update Information", "💬", 100, false).then((paneEl) => {
            ...
            fireAndForget(() => MarkdownRenderer.render(this.plugin.app, updateInformation, informationDivEl, "/", this.plugin));
        });
        ...
        void addPane(containerEl, "Setup", "🧙‍♂️", 110, false).then((paneEl) => {
            ...
            void addPanel(paneEl, "To setup the other devices", undefined, visibleOnly(() => this.isConfiguredAs("isConfigured", true))).then(paneEl => {
                new Setting(paneEl)
            ...
            void addPanel(paneEl, "Online Tips").then((paneEl) => {
                ...
                const troubleShootEl = this.createEl(paneEl, "div", {
                    text: "", cls: "sls-troubleshoot-preview"
                });
        ...
        void addPane(containerEl, "General Settings", "⚙️", 20, false).then((paneEl) => {
            void addPanel(paneEl, "Appearance").then((paneEl) => {
                const languages = Object.fromEntries([
                    ["", "Default"], ...SUPPORTED_I18N_LANGS.map(e => [
                        e, $t(`lang-${e}`)])]) as Record<I18N_LANGS, string>;
                ...
                new Setting(paneEl).autoWireToggle("showOnlyIconsOnEditor", { onUpdate: visibleOnly(() => this.isConfiguredAs("showStatusOnEditor", true)) });
...
`;
                    ...
                    new Setting(paneEl).autoWireText("secretKey", {
                        holdValue: true, isPassword: true
                    });
                ...
                void addPanel(paneEl, "CouchDB", undefined, onlyOnCouchDB).then(paneEl => {
                    ...
                    this.createEl(paneEl, "div", { text: `These settings are kept locked while any synchronization options are enabled. Disable these options in the "Sync Settings" tab to unlock.` }, undefined, visibleOnly(() => isAnySyncEnabled())).addClass("sls-setting-hidden");

                    new Setting(paneEl).autoWireText("couchDB_URI", {
                        holdValue: true, onUpdate: enableOnlySyncDisabled
                    });
            ...
            void addPanel(paneEl, "Confidentiality").then((paneEl) => {
                ...
                new Setting(paneEl)
                    .autoWireText("passphrase", {
                        holdValue: true, isPassword: true, onUpdate: isEncryptEnabled
                    })

                new Setting(paneEl)
                    .autoWireToggle("usePathObfuscation", {
                        holdValue: true, onUpdate: isEncryptEnabled
                    })
                new Setting(paneEl)
        ...
        void addPane(containerEl, "Sync Settings", "🔄", 30, false).then((paneEl) => {
            ...
            void addPanel(paneEl, "Synchronization Preset").then((paneEl) => {
                ...
                this.addOnSaved("preset", async (currentPreset) => {
                    ...
                    const presetLiveSync = {
                        ...presetAllDisabled, liveSync: true
                    } as Partial<ObsidianLiveSyncSettings>;
                    ...
                    if (currentPreset == "LIVESYNC") {
                        ...
                        this.editingSettings = {
                            ...this.editingSettings, ...presetPeriodic
                        }
            ...
            void addPanel(paneEl, "Hidden files", undefined, undefined, LEVEL_ADVANCED).then((paneEl) => {
                ...
                new Setting(paneEl)
                    .setClass("wizardHidden")
                    .autoWireToggle("syncInternalFilesBeforeReplication", { onUpdate: visibleOnly(() => this.isConfiguredAs("watchInternalFileChanges", false)) })

                new Setting(paneEl)
                    .setClass("wizardHidden")
                    .autoWireNumeric("syncInternalFilesInterval", {
        ...
        void addPane(containerEl, "Selector", "🚦", 33, false, LEVEL_ADVANCED).then((paneEl) => {
            void addPanel(paneEl, "Normal Files").then((paneEl) => {
                ...
                new MultipleRegExpControl({
                    target: syncFilesSetting.controlEl, props: {
                ...
                new MultipleRegExpControl({
                    target: nonSyncFilesSetting.controlEl, props: {
            ...
            void addPanel(paneEl, "Hidden Files", undefined, undefined, LEVEL_ADVANCED).then((paneEl) => {
                ...
                new MultipleRegExpControl({
                    target: patSetting.controlEl, props: {
                        patterns: pat,
                        originals: [...pat],
                        apply: async (newPatterns: string[]) => {
...
${stringifyYaml(pluginConfig)}`;
            ...
            void addPanel(paneEl, "Recovery and Repair").then((paneEl) => {
                ...
                new Setting(paneEl)
                    ...
                    .addButton((button) => button.setButtonText("Recreate all")
                        .setCta()
                        .onClick(async () => {
                ...
                new Setting(paneEl)
                    ...
                    .setDesc("Resolve all conflicted files by the newer one. Caution: This will overwrite the older one, and cannot resurrect the overwritten one.")
                    .addButton((button) => button.setButtonText("Resolve All")
                        .setCta()
                        .onClick(async () => {
                    ...
                    .addButton((button) => button
                        ...
                        .onClick(async () => {
                            ...
                            const entriesToDelete = (await this.plugin.localDatabase.allDocsRaw({
                                startkey: "ix:", endkey: "ix:\u{10ffff}", include_docs: true
                            }));
        ...
        void addPane(containerEl, "Advanced", "🔧", 46, false, LEVEL_ADVANCED).then((paneEl) => {
            ...
            void addPanel(paneEl, "Transfer Tweak").then((paneEl) => {
                ...
                new Setting(paneEl)
                    .setClass("wizardHidden")
                    .autoWireNumeric("concurrencyOfReadChunksOnline", {
                        clampMin: 10, onUpdate: onlyOnCouchDB
                    })

                new Setting(paneEl)
                    .setClass("wizardHidden")
                    .autoWireNumeric("minimumIntervalOfReadChunksOnline", {
                        clampMin: 10, onUpdate: onlyOnCouchDB
                    })
                // new Setting(paneEl)
                //     .setClass("wizardHidden")
        ...
        void addPane(containerEl, "Power users", "💪", 47, true, LEVEL_POWER_USER).then((paneEl) => {
            ...
            void addPanel(paneEl, "CouchDB Connection Tweak", undefined, onlyOnCouchDB).then((paneEl) => {
                ...
                new Setting(paneEl)
                    .setClass("wizardHidden")
                    .autoWireNumeric("batches_limit", {
                        clampMin: 2, onUpdate: onlyOnCouchDB
                    })
                new Setting(paneEl)
                    .setClass("wizardHidden")
            ...
            void addPanel(paneEl, "Configuration Encryption").then((paneEl) => {
                ...
                new Setting(paneEl)
                    ...
                    .autoWireDropDown("configPassphraseStore", {
                        options: passphrase_options, holdValue: true
                    })
                    .setClass("wizardHidden");
            ...
            void addPanel(paneEl, "Developer").then((paneEl) => {
                new Setting(paneEl)
                    .autoWireToggle("enableDebugTools")
                    .setClass("wizardHidden")
            });
        });
        ...
        void addPane(containerEl, "Maintenance", "🎛️", 70, true).then((paneEl) => {
            ...
            this.createEl(paneEl, "div", {
                ...
                this.createEl(c, "button", {
                    text: "I'm ready, mark this device 'resolved'", cls: "mod-warning"
                }, (e) => {
                    e.addEventListener("click", () => {
                        fireAndForget(async () => {
            ...
            void addPanel(paneEl, "Data-complementary Operations").then((paneEl) => {
                    ...
                    .addButton((button) => button
                        ...
                        .onClick(async () => {
                            await this.getMinioJournalSyncClient().updateCheckPointInfo((info) => ({
                                ...info, receivedFiles: new Set(), knownIDs: new Set()
                            }));
                ...
                new Setting(paneEl)
                    ...
                    .addButton((button) => button
                        ...
                        .onClick(async () => {
                            await this.getMinioJournalSyncClient().updateCheckPointInfo((info) => ({
                                ...info, lastLocalSeq: 0, sentIDs: new Set(), sentFiles: new Set()
                            }));
            ...
            void addPanel(paneEl, "Niches").then((paneEl) => {
                new Setting(paneEl)
                    ...
                    .addButton((button) => button.setButtonText("DryRun")
                        .setDisabled(false)
                        .onClick(async () => {
                            await this.dryRunGC();
                        })).addButton((button) => button.setButtonText("Perform cleaning")
                            .setDisabled(false)
                            .setWarning()
                            .onClick(async () => {
                                this.closeSetting()
    ...
    getMinioJournalSyncClient() {
        ...
        return new JournalSyncMinio(id, key, endpoint, bucket, this.plugin.simpleStore, this.plugin, useCustomRequestHandler, region);
    }
