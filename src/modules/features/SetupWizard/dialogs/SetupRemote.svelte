<script lang="ts">
    ...
    let proceedTitle = $derived.by(() => {
        if (userType === TYPE_COUCHDB) {
            return "Ui.SetupWizard.SetupRemote.ProceedCouchDb";
        } else if (userType === TYPE_BUCKET) {
            return "Ui.SetupWizard.SetupRemote.ProceedBucket";
        } else if (userType === TYPE_P2P) {
            return "Ui.SetupWizard.SetupRemote.ProceedP2P";
        } else {
...
<DialogHeader title="Ui.SetupWizard.SetupRemote.Title" />
<Instruction>
    <Question>Please select the type of server to which you are connecting.</Question>
    <Options>
        <Option selectedValue={TYPE_COUCHDB} title="Ui.SetupWizard.SetupRemote.CouchDbOption" bind:value={userType}>
            {msg("Ui.SetupWizard.SetupRemote.CouchDbOptionDesc")}
        </Option>
        <Option selectedValue={TYPE_BUCKET} title="Ui.SetupWizard.SetupRemote.BucketOption" bind:value={userType}>
            {msg("Ui.SetupWizard.SetupRemote.BucketOptionDesc")}
        </Option>
        <Option selectedValue={TYPE_P2P} title="Ui.SetupWizard.SetupRemote.P2POption" bind:value={userType}>
