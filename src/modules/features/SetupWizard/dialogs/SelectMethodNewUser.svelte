<script lang="ts">
    ...
    let proceedTitle = $derived.by(() => {
        if (userType === TYPE_USE_SETUP_URI) {
            return msg("Ui.SetupWizard.SelectNew.ProceedSetupUri");
        } else if (userType === TYPE_CONFIGURE_MANUALLY) {
            return msg("Ui.SetupWizard.SelectNew.ProceedManual");
        } else {
...
<DialogHeader title={msg("Ui.SetupWizard.SelectNew.Title")} />
<Guidance>{msg("Ui.SetupWizard.SelectNew.Guidance")}</Guidance>
<Instruction>
    <Question>{msg("Ui.SetupWizard.SelectNew.Question")}</Question>
    <Options>
        <Option selectedValue={TYPE_USE_SETUP_URI} title={msg("Ui.SetupWizard.SelectNew.SetupUriOption")} bind:value={userType}>
            {msg("Ui.SetupWizard.SelectNew.SetupUriOptionDesc")}
        </Option>
        <Option
            selectedValue={TYPE_CONFIGURE_MANUALLY}
            title={msg("Ui.SetupWizard.SelectNew.ManualOption")}
            bind:value={userType}
        >
            {msg("Ui.SetupWizard.SelectNew.ManualOptionDesc")}
        </Option>
    </Options>
</Instruction>
<UserDecisions>
